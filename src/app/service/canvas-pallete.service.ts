...
import { ColorI } from '../interface/canvas.interface';

@Injectable({
  providedIn: 'root'
})
export class CanvasPalleteService {
  ...
  materialColors:{[k:string]:{[k:string]:string}} = {
  ...
  pixelNegative(imageData: ImageData, i: number, color: ColorI, brightness:number = 255){
    imageData.data[i] = brightness - color.r;
    imageData.data[i+1] = brightness - color.g;
    imageData.data[i+2] = brightness - color.b;
  }

  giveRandomCartoonColors() :string[]{
  ...
  giveRgbShades(color:string) : string[]{
    let shades = []
    for(let shade in this.materialColors[color]){
      let s = this.materialColors[color][shade];
      shades.push(this.hexToRgb(s));
    }
    return shades;
  }
